...
public interface CommentRepository extends JpaRepository<Comments, UUID> {
  ...
  //
  @Query(value = "SELECT channels.profile_url AS channelProfileUrl, channels.name AS channelName, comments.content AS content, comments.create_at AS createAt, INSERT(INSERT(INSERT(INSERT(HEX(comments.comment_id), 9, 0, '-'), 14, 0, '-'), 19, 0, '-'), 24, 0, '-') AS commentId, comments.group_index AS groupIndex, childCount\n" +
  ...
  @Query(value = "SELECT channels.profile_url AS channelProfileUrl, channels.name AS channelName, comments.content AS content, comments.create_at AS createAt, INSERT(INSERT(INSERT(INSERT(HEX(comments.comment_id), 9, 0, '-'), 14, 0, '-'), 19, 0, '-'), 24, 0, '-') AS commentId, comments.group_index AS groupIndex\n" +
          "FROM comments \n" +
          "LEFT JOIN accounts\n" +
          "ON comments.account_id = accounts.account_id\n" +
          "LEFT JOIN channels\n" +
